def PyESPER_Mixed(
    ...
    *************************************************************************
    ...
    EstimatesLIR, UncertaintiesLIR, _ = PyESPER_LIR(
        DesiredVariables, Path, OutputCoordinates, PredictorMeasurements, **kwargs
    )
    ...
    for est_type in EstimatesLIR.keys():
        ...
        Uncertainties[est_type] = np.min(
            [uncertainties_lir, uncertainties_nn], axis=0
        ).tolist()

    toc = time.perf_counter()
    print(
        f"PyESPER_Mixed took {toc - tic:0.4f} seconds, or {(toc - tic) / 60:0.4f} minutes to run"
    )
